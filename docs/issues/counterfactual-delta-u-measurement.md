...
**Most likely cause, identified 2026-08-03: the refresh path poisons the shared cache.** `BatchedContextReader._read_one` calls `generate(prompt, refresh=True)` when a caller passes `reader_refresh`, and `LLMClient.chat` documents `refresh=True` as "skip the cache read, recompute, and **overwrite the cache file**". So one refreshed scoring mutates the content-addressed entry that every later reader shares. Two scorings of a byte-identical excerpt that straddle such an overwrite will legitimately differ even though both were themselves issued with `refresh=False` — which accounts for the 8 same-refresh cases as well as the 5 explicit mismatches, without needing to posit server-side non-determinism.

Rate is 0.015%, and for these cases the disagreement is itself the artifact — two scores from identical input means one is wrong and their difference is spurious. But it contradicts the determinism the reward pin is documented to rely on ("the determinism is load-bearing"), so it is filed rather than absorbed. Open: confirm the overwrite mechanism by replaying a refresh against a warm cache, and decide whether `refresh` should write to a side channel instead of mutating the shared entry.
