...
**Gate 1 — cheap, CPU, ~a day. Not yet run.** The representation gate: retrain the *existing* utility head as a menu-centered advantage regressor on breadth-first evidence, with policy-gradient, gain, hinge, sensitivity and count losses all disabled, split strictly by document. Question: can the current representation support the new role at all? If not, nothing downstream matters.
