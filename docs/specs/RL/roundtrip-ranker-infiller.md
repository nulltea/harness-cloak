...
| extractor                            | default `invert(out_p, R)`; opt-in frozen `extract(doc_p, R, out_p)` | legacy rule exact/fuzzy-90 cascade remains the default reward pin. Frozen extractor results carry `extractor_version`; cached rewards are valid only under the extractor pin they were produced with                                                                                                                                                                                                                       |
...
def R_rt(doc, actions):
    ...
    out_f  = invert(out_p, Rmap)                    # legacy default pin; frozen extractor is opt-in and keyed by extractor_version
