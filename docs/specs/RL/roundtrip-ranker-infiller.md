...
| probe teacher + atomic decomposition | writes probe questions | **Qwen3.6-35B-A3B**, non-thinking (escalated from LFM2.5 on 2026-07-05 via the pre-registered question-quality trigger: probe-health measured LFM questions as the binder — ambiguous multi-answer phrasing, mistargeted answers, reader abstention; ceiling pass ~23–30%, all docs under the 3-probe floor). Different family than the reward model ✓. One-time, cached; teacher-tagged cache auto-retires other-teacher entries |
