...
## Round-3 adjudication — evidence-supervised tie ownership (2026-07-31)
...
**Preregistered spike (awaiting approval).** Arms per Codex: A = online evidence-margin controller (labels one cycle lagged); B = cycle-projected variant (freeze residual during a cycle, accumulate evidence, small full-batch controller projection at cycle end — isolates whether timescale mismatch remains load-bearing after evidence routing). No-gap trio as frozen control; the hard lexicographic oracle computed from the same evidence for free as a ceiling and evidence-sufficiency diagnostic. 4 docs, seeds 17/29/47, 12 epochs, synchronous snapshots. Gates: greedy Delta P(lambda-3) >= 0.20 holding over the final three snapshots with no all-KEEP collapse and nondecreasing greedy privacy across profiles (sampled separation report-only); lambda-zero loss <= 0.044 every document; median frontier regret <= 0.044; >= 90% of evidence-qualified tie constraints satisfying the 0.1 greedy margin; no mislabeled costly pairs; nonzero cross-decision gain spread; ranges <= 50; evaluation without evidence override. Early kills: zero hinge gradient after cycle 1; gain spread < 1e-3 after cycle 2 despite >= 5 qualified decisions; two consecutive greedy lambda-3 collapses; any lambda-zero loss > 0.044; effective gain > 12 with constraints unsatisfied. Selection: A if both pass (simpler); B if only projection kills oscillation; document-utility failure with satisfied ties -> S-2; unsatisfied ties -> monotone action-conditioned increments; oracle itself failing -> evidence insufficient/non-compositional and the controller is no longer the root cause.

## Round-4 — held-out tie ownership: equivalence critic formalized from literature (2026-07-31)

**Inputs.** The evidence-supervised tie-ownership screening completed (RL-ranker v14, cycle projection adopted — oracle tracked at 43/48 doc-epochs, zero constraint violations, global alpha frozen; residual oscillation moved upstream into tower churn among ties plus evidence-coverage gaps), a Codex Sol High brainstorm round on ties-by-design, the consolidated spec (docs/specs/RL/ties-by-design.md — reward-tie semantics, counterfactuals-as-sensor, BC/ExIt margin installation, the coarse-side utility cliff, the deployment-generalization gap), and three targeted literature sweeps (11 papers registered) formalizing the recommended solution's three open design choices.

**The fork.** Tie ownership on training documents is per-document evidence; the ledger is empty at inference, so on unseen documents high-lambda tie-breaking degenerates to calibrated global alpha racing arbitrary interpolated tower margins — the original coin flip, invisible to utility regret (ties are utility-free) but material to realized privacy (~39% of measured decisions at or below the 0.044 reader floor). Solution ranking adopted in the spec: (i) counterfactual utility-equivalence critic — primary; (ii) tower tie-equality regularization — ablation only (uncalibrated logits, lambda-zero softening, live-pair interference); (iii) evidence-supervised gain head — comparison arm (models compensation, not equivalence); (iv) inference-time probing — audit oracle only (cost + cross-query privacy exposure); (v) accept — rejected (non-reproducible realized privacy invalidates the matched-privacy comparison).

**Literature-formalized design (full citations in the spec's fork section).** Loss: epsilon-insensitive Huber on the pairwise difference with epsilon = 0.044 — the regression analogue of the Rao-Kupper comparator-resolution threshold, a direct assignment of the measured reader floor with no fitting; rejects both plain Huber (fits signed sub-noise, degeneracy attractor at zero) and pure tie likelihoods (discard magnitudes, unanchored scale; label-forcing bias ~45-65% per Liu et al. does not apply to regression on measured Delta-U). Architecture: hurdle with the gate at 0.044 (classes ~39/61, no rebalancing needed — the 18% minority stratum was an artifact of splitting at zero); logistic tie gate + self-masking magnitude head on a frozen policy trunk; Balanced MSE as the structure-free baseline ablation. Deployment gate: frozen-rule certification — document-disjoint train/calibration/certification splits, accept-conditional false-equivalence rate bounded by exact one-sided Clopper-Pearson with the DOCUMENT as the certification unit; the finite-sample correction is itself preregistered: at realistic accepted counts (n 100-300) a certified 5%-risk/90%-confidence gate demands empirical precision 97-98%, and writing "95%" as the empirical bar would be a quiet calibration-trick error. Exchangeability caveat preregistered verbatim; the failure mode under shift is abstention (ordinary controller), never confident false ties.

**Preregistered screening (RL-ranker v15, awaiting approval — record: research-wiki/experiments/2026-07-31-RL-ranker-v15-equivalence-critic.md).** Offline supervised screening on the frozen adopted v14 cycle checkpoint; no RL training. Step 0 feasibility count (corpus-wide cache-mined pairs + v14 ledger; kill if certification cannot yield >= 45 accepted pairs). Arms: hurdle critic primary; epsilon 0 vs 0.044 ablation (the comparison the literature lacks); Balanced MSE baseline; CQR-interval acceptance only if the gate score is miscalibrated. Behavioral leg: certified gate bolted onto the frozen checkpoint, canonicalization at lambda > 0, exact identity at lambda = 0, synchronous snapshots on held-out documents. Gates and kills as in the spec's adjudication frame.

**Round-4 adjudication (2026-07-31, RL-ranker v15 complete).** The screening ran same-day on the frozen v14 cycle checkpoint (implementation codex-reviewed, 10 findings fixed pre-adjudication — the blocker being the document-level certificate, now the primary gate). Outcome: **decisive negative — no arm certifies.** Held-out gate AUC 0.60; the calibration-frozen threshold accepts 7/147 certification pairs with 4 violations (empirical precision 0.43 vs the corrected 0.98 bar); the min-accepted kill fires; the Balanced-MSE baseline sits at chance (AUC 0.49) so the gate-head deletion rule does not fire; the behavioral leg refused per preregistration. The conservative construction behaved as designed (≈95% abstention, no false-tie flood — the benign failure mode), but confident acceptances on unseen documents are wrong more often than right: with 97.6% of evidence rows on the 4 training documents, the critic's confident region is a 4-document neighborhood. Standing conclusion for the fork: feature-interpolated equivalence does NOT transfer under the current evidence distribution with frozen-feature linear probes. Open follow-ups, in order of leverage: (1) document-diverse evidence — cheap cache-only probing spread across the corpus, then rerun this exact protocol; (2) richer/shared-trunk pair features (also the only way to make the epsilon loss-form ablation consequential — under linear probes the gate and magnitude heads share nothing and the ablation was inert by construction); (3) fall back to evidence-supervised gain generalization or documented per-domain limits. Record: research-wiki/experiments/2026-07-31-RL-ranker-v15-equivalence-critic.md.

## Round-4 zoom-out — root-cause reassessment after the v15 negative (2026-08-01)

**Trigger.** Timo's challenge that the equivalence critic was "a sketchy workaround, a bolt-on component instead of fixing the root cause", plus a request to systematize scope, prevention, and principled alternatives that REASSESS existing components rather than adding new ones.

**Inputs.** Four independent passes: this session's codebase analysis, an independent Codex Sol High scope/prevention/reassessment pass, and three literature sweeps (scale identifiability; additive vs constrained/lexicographic composition; equivalence generalization and measurement-resolution limits) registering 13 new papers. Plus one new cache-only measurement (Gate 0).

**Three defects, not one.** (1) SCALE: ordinal training data identifies reward only up to positive linear scaling (Skalse 2023 Thm 3.10), so the tower's margins have no units by construction — every scale-control arm from v9 onward was unwinnable. (2) COMPOSITION: additive scalarization applied to a stratum where the primary gradient vanishes; only a set-level operation can localize such a decision. (3) MEASUREMENT (new, measured): counterfactual probes difference DOCUMENT-level utility although the artifact declares per-assertion policy dependencies. On 2,895 cached pairs whose decision has zero linked assertions — provably equivalent — the current statistic reports nonzero for 1,873 (65%), median 0.019. Restricting to linked assertions collapses the sub-noise band ~75% (train 4,735 -> 1,182 rows; certification 43 -> 14), grows the exact-tie core (3,010 -> 4,974), and sharpens the low-range separation from a 4.6x to a 38.5x jump. A sign-consistency test on the removed residue finds 62% noise-like versus 18% systematic; the systematic minority is filed as docs/issues/qa-dependency-underdeclaration.md and the conservative rule (both statistics below floor before labelling a tie) adopted.

**Citation autopsy.** The refutation was already on file and was read as motivation for an increment: Skalse 2022 describes a filter and we built a bonus; Tercan 2024's argument against a global threshold condemns a global exchange rate harder; Calvo-Fullana 2021's impossibility for any FIXED weighting was used to motivate a learned gain rather than to retire fixed-alpha, and v13's flat gain field is the predicted failure of building the state-augmented architecture without dual dynamics; Roy 2021 re-read shows the required multiplier is UNBOUNDED on a maximally-slack constraint, so the softcap adopted to preserve controller authority guaranteed the tie race would be lost; Vamplew 2024's prescription (deterministic, stated tie-breaking) IS a filter and was dropped while its diagnosis was kept.

**The critic, reassessed.** Not a workaround in function — a learned per-instance filter generalizing to unseen states is the published standard (Zahavy 2018) — but wrong in two specifics: the set rule must use a confidence bound rather than a point estimate, and the guarantee must be one-sided (never eliminate an action that could be optimal; abstain otherwise). v15 also certified an absolute-threshold object (verifiable regime) while deploying a distance-from-predicted-max set (unverifiable regime). And the flat-reward regime is precisely where learned equivalence metrics are documented to collapse (Kemertas 2021), so AUC 0.60 was forecastable rather than a tuning miss.

**Revised ranking (supersedes the round-3 fork ranking; full detail in docs/specs/RL/ties-by-design.md section 7).** (1) epsilon-lexicographic filter over anchored margins, shipping the exact-tie core first because it is provably free and, post-correction, larger and cleaner than believed; (2) anchored additive as the soft relaxation of the same object, admissible iff a cheap measurable separation test passes; (3) structural derivation as a constraint and a source of 2,895 free document-diverse labels across 23 documents, never as a deployment oracle; (4) full state-augmented Lagrangian only if (1) fails. DELETE: evidence hinge and cycle projection (a margin constraint inside the likelihood measured zero gain in Wang 2024), utility-logit softcap, capped learned gain, gap-scaling, KL anchor and sensitivity regularizer as training pressures. REPURPOSE: tower -> utility-advantage estimator anchored in delta-U units (HL-Gauss, sigma pinned to the measured floor); counterfactuals -> regression targets on linked assertions; lambda profiles -> document-level slack budgets; sensitivity -> evaluation-only.
