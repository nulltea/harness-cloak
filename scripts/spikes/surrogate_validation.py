...
def _pearson(xs, ys):
    ...
    return (
        cov / (vx * vy) if vx > 1e-9 and vy > 1e-9 else None
    )  # None = degenerate (flat)
...
def main():
    ...
    for corpus in args.corpora:
        ...
        arms_of = {
            d["id"]: build_arms(d["text"], det.detect(d["text"]), args.tau)
            for d in docs
        }
        probes = probes_for_docs(
            docs, {i: a["tau_walk"][1] for i, a in arms_of.items()}, workers=6
        )
        print(
            f"[{corpus}] {len(docs)} docs, probes/doc="
            f"{mean(len(probes[d['id']]) for d in docs):.1f} {time.time() - t0:.0f}s",
            flush=True,
        )

        jobs = [(d, arm) for d in docs for arm in ARMS]
        outs = pmap(
            lambda j: remote.generate(
                template.format(doc=arms_of[j[0]["id"]][j[1]][0])
            ),
            jobs,
        ...
        sc = score_batch(
            finals, [refs_of(d) for d, _ in jobs], use_bertscore=args.bertscore
        )
        rows = []
        ...
        print(
            f"[{corpus}] round trips + surrogate done {time.time() - t0:.0f}s",
            flush=True,
        )
        ...
        per_doc = {
            f"{g}~{s}": {"mean": round(mean(v), 3) if v else None, "n": len(v)}
            for (g, s), v in rhos.items()
        }
        for a in ARMS:
            ar = [r for r in rows if r["arm"] == a]
            frs = [
                r["realized_factrecall"]
                for r in ar
                if r["realized_factrecall"] is not None
            ]
            arm_means[a] = {
                "realized_factrecall": round(mean(frs), 4) if frs else None,
                "realized": round(mean(r["realized_rougeL"] for r in ar), 4),
                "u_surr": round(
                    mean(r["u_surr"] for r in ar if r["u_surr"] is not None), 4
                ),
                "u_qa": round(mean(r["u_qa"] for r in ar if r["u_qa"] is not None), 4)
                if any(r["u_qa"] is not None for r in ar)
                else None,
                "u_nli": round(
                    mean(r["u_nli"] for r in ar if r["u_nli"] is not None), 4
                ),
            }
        ...
        print(
            f"[{corpus}] arm means (factrecall|rougeL|u_surr): "
            + " ".join(
                f"{a}={arm_means[a]['realized_factrecall']}|{arm_means[a]['realized']}"
                f"|{arm_means[a]['u_surr']}"
                for a in ARMS
            )
            + f"  per-doc arm-spearman={per_doc}",
            flush=True,
        )
