...
Design: docs/plans/2026-07-05-extractor-inverse-designs.md
"""
import hashlib
import inspect
import json
...
def invert_implementation_pin() -> dict:
    ...
    implementation = {
        ...
        # Keys are the live module names, derived not typed: a package move changes them,
        # and changing them must change the pin (the cache is re-baselined, never faked).
        "modules": {
            __name__: _module_source_hash(sys.modules[__name__]),
            runtime_types.__name__: _module_source_hash(runtime_types),
        },
        "packages": {
...
def _rule_prepass(out_p: str, R: list[dict], *, semantic: bool) -> tuple[str, dict, list[dict]]:
    ...
    for repl, entries in sorted(gen.items(), key=lambda kv: -len(kv[0])):  # longest first
        # R is injective (detection/span_prep.py used-set): one replacement -> one surface; entries
...
if __name__ == "__main__":
    ...
    print("reward/extract.py self-check OK")
