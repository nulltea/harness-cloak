...
def pii_match_scores(entities: list[str], target: str, *, embed: Embed,
    ...
    `embed` is caller-supplied: the qwen3-embedding default died with the RANTEXT core
    (`inferdpt.embeddings`); `simcse_embed` below is the in-repo scorer.
    """
    if not entities:
