...
- **Adjudication of the single arms: both fail, on exactly complementary halves** — softcap = bounds without ownership; sensitivity = ownership without bounds. The pre-declared composition trigger is met: the composed arm (softcap 25 + sensitivity 0.1) is queued on seeds 47 and 17 behind the running softcap-17 chain; the moot sensitivity-17 single arm was cancelled.
