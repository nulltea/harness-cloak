...
- Supplementary (non-gated): a zombie queue instance ran composed-s17 concurrently with other chains (see kill-verification memory; results deterministic and valid). Cross-seed confirmation of the composed failure in a second presentation: early separation (+0.29, greedy 0.18/0.52) decays monotonically to profile MERGER by epoch 7 (greedy 0.08/0.08, Delta P +0.03) — s47 oscillates, s17 converges to indifference; both are shared-tower shape churn defeating a global controller. Its lambda-zero control run was cancelled as valueless for a rejected arm.
