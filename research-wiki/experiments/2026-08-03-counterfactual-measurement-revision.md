...
**R1 — targeted context re-scoring (A2+A3). IMPLEMENTED 2026-08-03** as `deduplicate_reader_work` in `src/cloak/reward/roundtrip.py`, applied in `score_roundtrip_batch`.

*Withdrawn and reinstated — record of the error.* This item was briefly marked "not built" on the argument that the content-addressed LLM cache already deduplicates 89.8% of context-reader calls (111,855 logical → 11,459 distinct prompts across the campaign). That argument was wrong for three reasons. The 89.8% is a **retrospective property of an accumulated cache**, not of the algorithm: it is zero on a cold cache and gives nothing for documents never scored, which is exactly the breadth-buying regime Gate 1 says we need. The cache is **mutable and unreliable** — the same session had just filed a defect showing the `refresh` path overwrites shared entries and produced 13 divergent values, so making cost and correctness depend on its state is unsound. And each redundant call is an **additional opportunity to pick up a divergent answer**, so not issuing it is a correctness benefit, not only a cost one. Dilution, by contrast, is genuinely R2's job, not R1's.

*Implementation.* `_read_utility_work` is a function of exactly `(set_valued, question, context, reader_clause)` at temperature 0, so identical prompts within one batch are collapsed to one issued call and the answer is fanned back out. The helper returns `(representatives, plan)` and reconstruction preserves `work_queue` order, which downstream finalization requires. New metrics: `reader_work_items` is now the issued count and `reader_work_deduplicated` reports the saving. Expected reduction: **78% of context assertion-instances** corpus-wide, independent of cache state.

*(original specification)* For a probe at decision X, re-score only those context assertions whose excerpt differs between the two assembled documents; reuse the score for byte-identical excerpts. Selection is a string comparison on the already-assembled `doc_p`, requires no model, and is exact under `temperature=0`. Expected saving on the verified document: 9/14 (64%) of context reader calls. Note this subsumes the A2/A3 distinction — "span is subject/object" and "span merely occurs in the excerpt" both reduce to "the excerpt changed", which is the provable criterion.
